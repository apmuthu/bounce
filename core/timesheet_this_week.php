...
if($lw == null)
{
	...
	$lw = date("Y-m-d", $lessweek);
		...
		$weekly = mysql_query("SELECT * FROM timesheets WHERE techid = $tech_id AND daterec <= '$dates'  AND daterec > '$lw' ORDER BY daterec")or die (mysql_error());		
		while($row = mysql_fetch_array($weekly))
		{
          ...
          <td width="48" nowrap="nowrap" bgcolor="#FFFFFF" class="style2" align="center"><?php echo "$row[time]"; ?></td>
